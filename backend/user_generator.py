...
def generate_user_data(name: str = None):
    """Generate a complete fake user with accounts and transactions"""
    
    user_id = f"user_{uuid.uuid4()}"
    
    # Generate user
    user_name = name or fake.name() # Use provided name or fake one
    user = {
        'id': user_id,
        'name': user_name,
        'email': f"{str(uuid.uuid4())[:8]}-{fake.email(domain='demo.com')}",
    ...
    # Get checking and savings accounts for transfers
    ...
    for i in range(num_transactions):
        ...
        if transaction_type == 'payment':
            ...
            from_account = random.choice(checking_accounts + savings_accounts)
            ...
            to_account = random.choice(checking_accounts)
